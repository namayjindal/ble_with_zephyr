...
static struct calibration_data cal_data = {{0}};
...
void main(void)
{
    ...
    while (1) {
        if (sensor_sample_fetch(lsm6dsl_dev) < 0) {
            ...
            s_data.index = index;
            s_data.accel_x = accel[0];
            s_data.accel_y = accel[1];
            s_data.accel_z = accel[2];
            s_data.gyro_x = gyro[0];
            s_data.gyro_y = gyro[1];
            s_data.gyro_z = gyro[2];
