...
# Define your exercise configurations and sensor configurations
EXERCISE_CONFIGS = {
...
csv_filename = "2_all_sensors_with_pause.csv"
...
async def connect_to_sensors(sensor_ids):
    ...
    for sensor_id in sensor_ids:
        ...
        if device:
            connected_sensors.append((device, config, sensor_id))
        else:
...
async def start_data_collection(connected_sensors):
    tasks = [connect_and_run(device, config, sensor_id) for device, config, sensor_id in connected_sensors]
...
async def main():
    ...
    connected_sensors = await connect_to_sensors(sensor_ids)
    if not connected_sensors:
        logger.error("No sensors found for the selected exercise.")
        return

    print(f"Connected to sensors: {[config['name'] for _, config, _ in connected_sensors]}")
    proceed = input("Proceed with data collection? (y/n): ").lower()
    if proceed != 'y':
        logger.info("Data collection aborted by user.")
        return

    sensor_data = SensorData(sensor_ids)
    create_csv_file(sensor_ids)

    await start_data_collection(connected_sensors)
