...
lib = ctypes.CDLL('./build/libmyapp.so')

# Define the function prototype
lib.process_data.argtypes = [ctypes.c_char_p, ctypes.c_int, ctypes.c_double, ctypes.c_int]
...
SENSOR_CONFIGS = {
    ...
    5: {
        "name": "Sense Ball",
        "service_uuid": "9E400001-C5C3-E393-A0F9-F50E24DCCA9E",
        "char_uuid": "9E400002-C5C3-E393-A0F9-F50E24DCCA9E",
        "reference_uuid": "9E400003-C5C3-E393-A0F9-F50E24DCCA9E",
        "prefix": "ball"
    }
}
...
EXERCISE_CONFIGS = {
    ...
    "hands_and_ball": [3, 4, 5],  # All sensors
    "all_sensors": [1, 2, 3, 5],  # All sensors
...
class BLEWorker(QObject):
    ...
    async def write_calibration_trigger(self):
        calibration_trigger = struct.pack('<B', 1)
        for sensor_id, client in self.clients.items():
            config = self.sensor_configs[sensor_id]
            await client.write_gatt_char(config['reference_uuid'], calibration_trigger)
        logging.info(f"Written calibration trigger to all sensors")
    ...
    def notification_handler(self, sensor_id, sender, data):
        # Unpack the buffer size (assuming it's sent as the first byte)
        buffer_size = struct.unpack('<B', data[:1])[0]
        data = data[1:]  # Remove the buffer size byte

        # Calculate the size of each sensor_data struct
        sensor_data_size = struct.calcsize('<IIffffffB')

        # Unpack all sensor readings in the buffer
        for i in range(buffer_size):
            start = i * sensor_data_size
            end = start + sensor_data_size
            sensor_data = data[start:end]

            timestamp, index_value, accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z, battery_percentage = struct.unpack('<IIffffffB', sensor_data)
            timestamp_sec = timestamp / 1000
            values = [index_value] + [round(value, 4) for value in [accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z]] + [battery_percentage]
            
            # Emit the data for each sensor reading
            self.data_received.emit(sensor_id, timestamp_sec, values)

        logger.debug(f"Processed buffer of {buffer_size} readings for sensor {sensor_id}")
...
class ExercisePage(QWidget):
    ...
    def initUI(self):
        ...
        # self.calibrate_button = QPushButton("Calibrate Sensors")
        # self.calibrate_button.clicked.connect(self.calibrate_sensors)
        ...
        self.reset_button.clicked.connect(self.reset_exercise_page)
    ...
    #     await self.ble_worker.write_calibration_trigger()
    ...
    def process_file(self):
        self.peaks_label.setText("Processing...")
        QApplication.processEvents()  # Force UI update

        file_name_bytes = self.csv_filename.encode('utf-8')
        
        # Call the C++ function
        num_good_reps, num_anomalies = lib.process_data(file_name_bytes, 20, 0.6, 15)
        
        if num_good_reps:
            self.peaks_label.setText("Processing completed successfully!")
        else:
            self.peaks_label.setText(f"Processing failed!")

        print(f"Number of good reps : {num_good_reps}")
        ...
        QMessageBox.information(self, 'Processing Complete', 
                                f'Data has been processed from {self.csv_filename}\n'
                                f'Number of good reps: {num_good_reps}\n'
